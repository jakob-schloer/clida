...
ano_red_fname = dirname + "era5_sst_2000-2019_mon_anomalies.nc"
...
def animate(frame):
    ...
    ax.add_feature(ctp.feature.RIVERS)
    ax.add_feature(ctp.feature.BORDERS, linestyle=':')
    ax.set_title(str(da.time.data[frame])[:10])
    dmap = da.isel(time=frame)
    cont = plt.contourf(lons, lats, dmap, 60, cmap=cmap, vmin=vmin, vmax=vmax,
                    transform=ccrs.PlateCarree())
    return cont

ani = animation.FuncAnimation(fig, animate, frames=np.arange(0, da.shape[0]), interval = 2000, blit=False)
